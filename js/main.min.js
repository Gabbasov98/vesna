...
function fix100vh(){let e=.01*window.innerHeight;document.documentElement.style.setProperty("--vh",e+"px")}window.addEventListener("DOMContentLoaded",()=>{fix100vh(),window.addEventListener("resize",()=>{fix100vh(),getScrollBarSize()}),getScrollBarSize()});var $status=$(".pagingInfo"),$slickElement=$(".slick-slider");$slickElement.on("init reInit afterChange",(function(e,t,n,i){var s=(n||0)+1;$status.text(s+"/"+t.slideCount),$status.text(`<span class="любой-класс">${s}</span> / <span class="любой-класс">${t.slideCount}</span>`)}));
